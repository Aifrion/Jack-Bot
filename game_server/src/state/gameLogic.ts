...
export const robotRoomKey = (code: string) => `robot:${code}`;
...
function scheduleAdvance(room: Room, ms: number, next: () => void): void {
  clearRoomTimer(room);
...
function enterIntro(io: Server, room: Room): void {
  room.phase = 'intro';
...
function enterQuestion(io: Server, room: Room): void {
  if (room.currentQuestionIndex >= QUESTIONS.length) room.currentQuestionIndex = 0;
...
function enterAnnouncement(io: Server, room: Room): void {
...
function enterDefense(io: Server, room: Room): void {
  room.phase = 'defense';
  room.robotScript =
...
function enterVoting(io: Server, room: Room): void {
  room.phase = 'voting';
  room.votes = new Map();
  room.robotScript = null;
...
function enterRoundResult(io: Server, room: Room): void {
  clearRoomTimer(room);

  const tally = new Map<string, number>();
  for (const votedFor of room.votes.values()) {
    if (!room.alivePlayerIds.has(votedFor)) continue;
...
export function submitAnswer(io: Server, room: Room, socketId: string, answer: number): string | null {
  if (room.phase !== 'question') return 'Not in question phase';
  if (!room.alivePlayerIds.has(socketId)) return 'You are not alive in this round';
  if (!Number.isInteger(answer) || answer < 0 || answer > 10) return 'Answer must be an integer 0–10';
  room.answers.set(socketId, answer);
  broadcastRoomState(io, room);
  if (allAliveAnswered(room)) enterAnnouncement(io, room);
  return null;
}

export function submitVote(io: Server, room: Room, voterId: string, votedFor: string): string | null {
  if (room.phase !== 'voting') return 'Not in voting phase';
  if (!room.alivePlayerIds.has(voterId)) return 'You are not alive in this round';
  if (!room.alivePlayerIds.has(votedFor)) return 'Target is not alive';
  if (voterId === votedFor) return "You can't vote for yourself";
  room.votes.set(voterId, votedFor);
  broadcastRoomState(io, room);
  if (allAliveVoted(room)) enterRoundResult(io, room);
  return null;
}

export function advancePhase(io: Server, room: Room, callerSocketId: string): string | null {
  const isHost = callerSocketId === room.hostSocketId;
  const isRobot = room.robotSubscriberIds.has(callerSocketId);
  if (!isHost && !isRobot) return 'Only the host or a subscribed robot can advance phases';
  switch (room.phase) {
    case 'intro':
...
function allAliveAnswered(room: Room): boolean {
  for (const sid of room.alivePlayerIds) if (!room.answers.has(sid)) return false;
  return true;
}

function allAliveVoted(room: Room): boolean {
  for (const sid of room.alivePlayerIds) if (!room.votes.has(sid)) return false;
  return true;
}
